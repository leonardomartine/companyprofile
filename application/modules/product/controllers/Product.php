...
class Product extends App
{
	function __construct() {
		parent::__construct();
		$this->load->model(['ProductModel', 'ProductCategoryModel', 'ProductSubCategoryModel']);
	...
	public function category($category = null) {
		$entity = $this->ProductCategoryModel->getDetail('id',$category);

		$pagination = $this->setSubcategoryPagination();
		$data = array(
			'app' => $this->app($entity->name),
			'data' => $this->ProductSubCategoryModel->getAllPaginate([
				'product_category_id' => intval($category)
			], [], $pagination->config->per_page, $pagination->offset),
			'pagination' => $pagination->link
		);

		$this->template->set('title', $data['app']->active_module->name . ' | ' . $data['app']->app_name, TRUE);
		$this->template->load_view($data['app']->template_frontend.'/subcategory', $data, TRUE);
		$this->template->render();
	}

	public function subcategory($category = null, $subcategory = null) {
		$entity = $this->ProductSubCategoryModel->getDetail('id',$category);
		...
		$data = array(
			...
			'data' => $this->ProductModel->getAll([
				'product_category_id' => intval($category),
				'product_sub_category_id' => intval($subcategory),
			], [], $pagination->config->per_page, $pagination->offset),
			'data_category' => $this->ProductCategoryModel->getAll(),
			'pagination' => $pagination->link
		);
		...
		$this->template->set('title', $data['app']->active_module->name . ' | ' . $data['app']->app_name, TRUE);
		$this->template->load_view($data['app']->template_frontend.'/product', $data, TRUE);
		$this->template->render();
	}

	public function view($c=null,$s=null,$link = null) {
		...
		if (count($temp) == 1) {
			$data = array(
				'app' => $this->app(),
				'data' => $temp,
				'data_latest' => $this->ProductModel->getLatest(6, intval($c))
			);
	...
	private function setSubcategoryPagination() {
		$pagination = array(
			'per_page' => 12,
			'base_url' => base_url('product/'),
			'total_rows' => $this->ProductSubCategoryModel->getRowCount(),
			'use_page_numbers' => true,
			'page_query_string' => true,
			'query_string_segment' => 'page',
			'first_link' => 'First',
